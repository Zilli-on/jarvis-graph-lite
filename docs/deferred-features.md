...
## Promoted out of this file in v0.12.2

- **`# noqa: F401` suppression in `find_unused_imports`**. Dogfooding v0.12.1 on jarvis-graph-lite itself surfaced three "unused" imports in the test suite — all were `from conftest import ROOT  # noqa: F401  path setup` style imports that exist purely to trigger `conftest.py`'s `sys.path` side effect. Flake8 and ruff both honour the `# noqa: F401` directive; the engine didn't. v0.12.2 adds a third suppression path after the call-graph check and the textual token fallback: read the logical import line (joining physical continuation lines for the multi-line `from X import (\n  a,\n  b,\n)` form) and scan for `# noqa` directives. Blanket `# noqa`, `# noqa:` (empty), `# noqa: F401`, and mixed-code lists like `# noqa: E501, F401` all suppress. Specific non-F401 directives (`# noqa: E501`) do NOT suppress. The regex extracts valid codes with `\b[A-Z]{1,3}\d{3,4}\b` after greedy match so trailing commentary (`# noqa: F401  path setup`) is tolerated. Validation: jarvis-graph-lite's own `find_unused_imports` run now reports **0 unused imports out of 412** (was 3 false positives). The full test suite is now **270 passing** (+14: 8 directive unit tests, 3 multi-line helper tests, 3 end-to-end tests against a tiny tmp-dir repo with mixed noqa/plain imports).
